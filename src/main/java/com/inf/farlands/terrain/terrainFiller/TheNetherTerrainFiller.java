 ...
 */
public final class TheNetherTerrainFiller extends AbstractTerrainFiller {

    private TheNetherTerrainFiller(NoiseGeneratorSettings settings) {
        super(settings);
    }

    /** 从 generator 构造，由 GenQueue 惰性缓存持有。 */
    public static TheNetherTerrainFiller of(ServerLevel level) {
        NoiseBasedChunkGenerator gen = (NoiseBasedChunkGenerator) level.getChunkSource().getGenerator();
        return new TheNetherTerrainFiller(gen.generatorSettings().value());
    }

    @Override
    protected TerrainFillerContext.TerrainDimension dimension() {
        return TerrainFillerContext.TerrainDimension.NETHER;
    }

    @Override
    protected TerrainSystem terrainSystem() {
        return TerrainSystemRegistry.getTheNether();
    }
}
